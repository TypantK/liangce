...
def _sample_news(query: str) -> list[dict]:
    ...
    使用相对于当前日期的动态偏移，确保回测期总能匹配到情绪事件：
      day-5 ~ day-3: 利好密集，策略正常买入
      day-2:         利空初现
    ...
    return [
        # ---- day-5: 纯利好 ----
        {"title": f"机构看好{query}赛道：行业景气度持续提升", "snippet": f"日期: {d5} 多家券商发布{query}行业研报，评级上调至增持。", "url": _search_url(f"机构看好{query} 行业景气度"), "source": "sample-bull"},
        {"title": f"{query}龙头业绩超预期，盈利大幅增长", "snippet": f"日期: {d5} {query}龙头公布季报，净利润同比增长45%，远超预期。", "url": _search_url(f"{query} 龙头 业绩 超预期"), "source": "sample-bull"},
        ...
        # ---- day-4: 利好延续 ----
        {"title": f"{query}板块利好出台，多只个股涨停创新高", "snippet": f"日期: {d4} {query}板块受政策利好带动，多只成分股涨停，板块指数创年内新高。", "url": _search_url(f"{query} 板块 利好 涨停"), "source": "sample-bull"},
        {"title": f"北向资金大幅流入{query}板块，主力加仓信号", "snippet": f"日期: {d4} 北向资金今日净流入{query}板块超50亿，市场看多情绪浓厚。", "url": _search_url(f"北向资金 {query} 主力加仓"), "source": "sample-bull"},
        {"title": f"{query}签下重大海外订单，国际业务突破", "snippet": f"日期: {d4} {query}头部企业宣布与海外客户签订十年合作协议，出海战略加速落地。", "url": _search_url(f"{query} 海外订单 国际业务"), "source": "sample-bull"},
        # ---- day-2: 利空开始浮现 ----
        {"title": f"监管层关注{query}领域风险——短期利空需警惕", "snippet": f"日期: {d2} 监管部门就{query}行业发布风险提示函，涉及合规和数据安全问题。", "url": _search_url(f"监管 {query} 风险提示"), "source": "sample-bear"},
        ...
        {"title": f"{query}行业评级遭集体下调，多家机构看空后市", "snippet": f"日期: {d1} 受监管和供应链双重压力，多家券商下调{query}板块评级至减持。", "url": _search_url(f"{query} 评级下调 看空"), "source": "sample-strong-bear"},
    ]
